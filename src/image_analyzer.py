...
class ImageAnalyzer:
    ...
    def atoms_in_image(self, image_path: str) -> np.ndarray:
        ...
        for atom in self.atoms:
            if (
                pixel_values[
                    atom["position_y"] - self.roi_side_length // 2 : atom["position_y"]
                    - self.roi_side_length // 2
                    + self.roi_side_length,
                    atom["position_x"] - self.roi_side_length // 2 : atom["position_x"]
                    - self.roi_side_length // 2
                    + self.roi_side_length,
                ].sum()
                > atom["threshold"]
    ...
    def atoms_survival_ratio(
        self, ref_occupancy: np.ndarray, final_occupancy: np.ndarray
    ):
        ...
        if self.images_per_cycle == 2:
            ...
            for parameter in range(len(self.parameters)):
                ...
                if total_atoms_initial < total_atoms_final:
                    print(
                        f"total_atoms_initial < total_atoms_final for parameter {parameter}"
                    )
                    print("we skip this parameter")
                    continue
                total_atoms_initial_error = self.data[mask_initial][
                    "total_atoms"
                ].std() / np.sqrt(len(self.data[mask_initial]))
                total_atoms_final_error = self.data[mask_final][
                    "total_atoms"
                ].std() / np.sqrt(len(self.data[mask_final]))
                ratio = np.array(self.data[mask_final]["total_atoms"]) / np.array(
                    self.data[mask_initial]["total_atoms"]
                )  # total_atoms_final / total_atoms_initial
    ...
    def data_per_atom(self, atom_index: int):
        ...
        if self.numbering.images_per_cycle == 2:
            ...
            for parameter in range(len(self.numbering.parameters)):
                ...
                mask_final = (self.data["parameter_index"] == parameter) & final_image
                atom_presence_initial = (
                    self.data[mask_initial]["atoms_in_image"].str[atom_index].sum()
                )
                atom_presence_final = (
                    self.data[mask_final]["atoms_in_image"].str[atom_index].sum()
                )
                rows.append(
                    {
...
class plotter:
    ...
    def plot_survival_vs_parameter(self):
        ...
        for param_idx in range(self.n_parameters):
            ...
            ref_occupancy = self.images_data[
                mask & (self.images_data["image_type"] == 0)
            ]["atoms_in_image"].sum()
            final_occupancy = self.images_data[
                mask & (self.images_data["image_type"] == 1)
            ]["atoms_in_image"].sum()
    ...
    def plot_survival_per_atom(self, atom_index: int):
        ...
        for param_idx in range(self.n_parameters):
            ...
            for rep_idx in range(self.n_reps):
                ref_occupancy = self.images_data[
                    mask & initial & (self.images_data["repetition_index"] == rep_idx)
                ]["atoms_in_image"].tolist()
                final_occupancy = self.images_data[
                    mask & final & (self.images_data["repetition_index"] == rep_idx)
                ]["atoms_in_image"].tolist()
            ...
            survival_ratios.append(
                final_atom_presence / ref_atom_presence
                if ref_atom_presence > 0
                else None
            )
        self.plot_graph(
            self.parameters,
